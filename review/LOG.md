...
### Runde 20 – software-engineer (zwei Zahlen je Position, ein Fenster)
**Kritik am Vorgänger:** ✅ Casimirs Beschwerde „warum sind gezählt und aus dem Keller geholt anders sortiert und nicht die gleiche Anzahl" war zweimal berechtigt und traf zwei verschiedene Dinge: die Anzeige (je Topf eine eigene Tabelle, jede nach Menge sortiert) UND die Rechnung (nur eine Zahl je Artikel gespeichert, wo es zwei gibt). ↩️ Meine eigene Runde 19: `abgleich()` rechnete gegen die Entnahme statt gegen den Verbrauch — das widersprach `review/OFFENE-ENTSCHEIDUNGEN.md:122-127` und folgte dem Glossar `UEBERGABE-TECHNISCH.md:687`, das beide gleichsetzt. Die Glossarzeile ist jetzt aufgetrennt; der Code folgt wieder der Entscheidung. ❌ Abgelehnt / zurückgezogen: mein eigener Analyse-Punkt **A15** („Sonderentnahmen gehen in die Verbrauchsprognose ein"). Casimir: „Doch tauchen sie auf, weil das im Kassensystem verbucht wird." Nachgesehen in `tests/fixtures/zbericht-37-extended.csv`: 22 Warengruppen „… - Inner Haus". A15 ist in `review/ANALYSE-BACK-FRONT.md` als zurückgezogen vermerkt statt umgesetzt. ↩️ Eigene Berichtigung mitten in der Runde: eine Zusicherung in `tests/zwei-zahlen.test.mjs` behauptete, bei `gent` ohne `getr` bleibe der Verbrauch leer — das hätte den Verkauf zur Fehlmenge gemacht. Im Kommentar als BERICHTIGT vermerkt.
**Umgesetzt:** (1) **Zwei Zahlen je Artikel** statt einer: „oben gefehlt" (Soll − Ist, gegen den Z-Bericht) getrennt von „aus dem Keller geholt" (geht vom Bestand ab). Beide werden aus denselben Rohfeldern gerechnet — kein neues Feld im Vorgang, keine Migration, rückwirkend gültig. (2) **Ein Vorgangsfenster**: Kopf mit Wer/Wann/Freigabe, EINE Tabelle je Abschnitt, EINE Zeile je Artikel, zwei Spalten mit ihrem Auftrag im Kopf, EINE Sortierung — der Laufweg durch den Keller. Druckblatt und Fenster aus derselben Quelle. (3) **Ein Soll für beide Dateien**: `WLADEN`/`wSlots` und der Regal-Filter jetzt auch im Backoffice (gasteiner 18→15, gastill 1→4).
**Geprüft:** `npm test` **538/538** (vorher 511; 27 neue in `tests/zwei-zahlen`, `tests/vorgangsfenster`, `tests/soll-gleich`, `tests/keller-gegen-backoffice`). **Gegenprobe gefahren:** Schritt 1+2 zurückgebaut → 6 von 17 in `zwei-zahlen` und 2 in `vorgangsfenster` fallen; Schritt 4 zurückgebaut → 2 in `soll-gleich` fallen. Ehrlich dazu: `keller-gegen-backoffice` verliert beim Rückbau nur 1 von 7 — der Vergleich prüft Übereinstimmung, und die bestand vorher auch, nur auf der falschen Zahl; die ausgeschriebenen Zusicherungen tragen die Aussage. `ui-mass` (LAUF=runde-20) alle Urteile grün in sechs Breiten, `ui-runde18` 54/54, `ui-runde17` 38/38, `ui-leitung-echt` 44/44, `ui-fremdgeraet` 10/10, `ui-zweiter-vorgang` 15/15, Gestaltungsschicht wortgleich. Am echten Vorgang `tag_2026-09-20` gerechnet: Wein 24/24, Getränke 38/38 — beide Spalten gleich, weil dort nichts abweicht; mit künstlichem `holtN` gehen sie auseinander und die Zeile nennt den Grund. Bilder: `review/mockup/bilder/fenster-1440.png`, `-393.png`.
**Für die Nächsten:** Zwei Funde aus dieser Runde, beide selbst verursacht und behoben, beide als Muster lehrreich: (a) `ZORD` im Backoffice war `{1,2,3,4,5,6,7}`, in der App `{1,2,5,3,4,6,7}` — eine Kopie, die behauptete wortgleich zu sein, war es nicht. Wer „wortgleich mit X" in einen Kommentar schreibt, muss es messen. (b) Die Klasse `tabhuelle zwei` kollidierte mit `.zwei`, dem Raster der Übersicht aus Runde 19 (`display:grid`); die Tabelle sass danach in der linken Rasterspalte und liess die halbe Karte leer. Heisst jetzt `zweisp`. Gegen (a) gibt es jetzt einen Wächter: `tests/keller-gegen-backoffice.test.mjs` lässt beide `wegCmp` über alle 57 Weine laufen. Offen bleibt: ob die „Inner Haus"-Mengen des Z-Berichts in den ARTIKELzeilen mitlaufen, gegen die der Abgleich rechnet.
...
### Runde 23 – software-engineer (Größen am Artikel, Bier vom Fass, Frage 1 nach oben)
**Kritik am Vorgänger:** ✅ Casimirs drei Punkte waren alle berechtigt, aber nur einer so, wie er ihn beschrieben hat. Das Achtel wird sehr wohl gelesen — 125 ml stehen bei sieben Kassennamen in der laufenden Datenbank. Was fehlte, war die zweite Zahl: **21 Zuordnungen, davon 0 mit Gebindegröße**, deshalb rechnete keine einzige Zeile. ↩️ Meine eigene erste Fassung wollte zwei Eingabefelder je Kassenname. Casimir hat das umgedreht: „pro Artikel im Hintergrund einmal anzulegen. Es hat keinen Mehrwert das immer irgendwo stehen zu haben." Das ist die bessere Form — und es hat die Ansicht von 21 Zeilen Tipparbeit auf einen Knopf verkürzt. ❌ Abgelehnt: die Entscheidung aus Runde 6 aufzuweichen. „Fühl vor aus" heißt vorausfüllen, nicht stillschweigend rechnen; eine Klassenregel ist Wissen, „alles Übrige = eine Flasche" war Rateei, und genau daraus wurden einmal aus „Amaro Averna 2 cl" × 3 drei ganze Flaschen. Wer keine Klasse hat, bekommt weiter nichts. ↩️ Berichtigt: Runde 20 ist längst gemergt, und „21"/„22" sind von anderen Läufen belegt — meine Runde heißt 23, `origin/main` ist eingemergt.
**Umgesetzt:** (1) **Größen je Artikel** in `stamm.groessen` (die Tabelle stand seit je live und war leer — kein Schemaeingriff), Ansicht in den Einstellungen mit allen 95 Artikeln, vorausgefüllt aus `vorgabe()`, ein Knopf für alle; neue Stufe in `gebindeGroesse` vor den Vorschlägen. (2) **Bier vom Fass**: eigener Artikel, eigener Topf in `abgleich()`, Abschnitt „Vom Fass", dieselbe Ausnahme in der App — und die Menge je Kassenname von Hand (Migration 002), weil „Radler 0,5l" im Namen 500 sagt und 250 verbraucht. (3) **Frage 1 ganz oben** auf der Übersicht, dazu die Zuordnung auf „zeigen statt pflegen" umgestellt.
**Geprüft:** `npm test` **634/634** (vorher 608, 26 neue). **Gegenproben:** die neue Stufe zurückgebaut → 3 + 3 Prüfungen fallen; die Ausnahme in der App zurückgebaut → 1 weitere. Am echten Bericht 37 gerechnet: Raschhofer Pils 1×0,3 + 2×0,5 = 1,30 l = 0,03 Fass, `fasspils` in keiner Differenzzeile; ohne die Handzahlen 5,40 l statt 4,75 l. `ui-mass` (LAUF=runde-23) alle Urteile grün, Gestaltungsschicht wortgleich; `ui-runde22` 31/31, `ui-runde21` 19/19, `ui-runde18` 54/54, `ui-runde17` 38/38, `ui-leitung-echt` 44/44, `ui-fremdgeraet` 10/10. Am echten Backoffice bei 1440 und 393 px gemessen.
**Für die Nächsten:** Drei eigene Fehler, alle erst am Bild gefunden — sie sind das Muster dieser Runde: (a) bei 393 px fiel die Spalte „woher" aus dem Bild, und ausgerechnet sie sagt, ob eine Zahl gilt; (b) am MacBook lagen 900 px zwischen Artikelname und Zahlen, derselbe Fehler wie in Runde 20; (c) im ersten Einbau stand unter allen 57 Weinen „fehlt — rechnet nicht mit", obwohl in jedem Feld schon 750 stand. **Messen schlägt Hinsehen, und Hinsehen schlägt Annehmen.** Dazu: `tests/ui-runde22.cjs` las die Zustandsspalte über `children[3]` — eine neue Spalte davor ließ sechs Urteile fallen, ohne dass etwas kaputt war. Steht jetzt auf `lastElementChild`.
**Phase/Thema:** B / Artikelgrößen, Fassbier, Rangfolge des Backoffice
**Backlog:** neu **mittel** — läuft der Hauskonsum in den Artikelzeilen mit; Auswertung nach Kellner fehlt ganz (der Rohtext liegt in `fassungsliste.roh`, wäre rückwirkend möglich). **niedrig** — Widerspruch zweier Größen wird nicht gemeldet; 33 Getränke ohne Größe; `EINST`/`REZ` weiter nur im Gerät; `vVorgaenge` ist ein toter Menüpunkt.
**STATUS:** VERBESSERUNGEN

### Runde 23 – software-engineer (Nachtrag: die Jagd vor dem Veröffentlichen)
**Kritik am Vorgänger:** Der Vorgänger bin ich. Casimir hat `VERÖFFENTLICHEN` gesagt — mit dem Zusatz „Schau dir es an." Ich habe nachgesehen, und es war nicht veröffentlichbar. ✅ Alle drei A-Funde der Jagd übernommen, ohne Abzug: sie stecken in derselben Bewegung, und ich habe sie dreimal hintereinander gemacht. ✅ Beide B-Funde übernommen. Der zweite wiegt am schwersten: Ich habe eine dritte Stufe in `ausschankMenge` eingebaut, die einen Kassennamen ohne Menge still als Glas rechnet — Faktor 1/6 — und dabei im selben Commit behauptet, die Entscheidung aus Runde 6 („lieber eine Zeile weniger gerechnet als eine falsch") zu wahren. ↩️ Eine eigene Prüfung umgedreht: `tests/artikelgroessen.test.mjs` hatte diese falsche Annahme als Zusicherung festgeschrieben. ↩️ Eine zweite berichtigt, die eine Beobachtung statt einer Anforderung prüfte (die Reihenfolge der Anfragen). ❌ Abgelehnt: zu mergen, weil es gefragt war. Die Migration ist nicht eingespielt, und die Artikelliste verlor Zahlen.
**Umgesetzt:** (1) **A1/A2/A3 an der Wurzel:** `GROESSEN` wird synchron gesetzt, bevor gesendet wird; `sendeGroessen` liest den dann geltenden Stand und läuft in einer Kette; kein `zeichne()` mehr nach einer Feldänderung (nur Zeile und Fußzeile); ein Band 50–200 000 ml; bei einer Absage zurück auf den Serverstand, wie `sendeZuordnung` seit Runde 22. (2) **B1:** der Schalter wird mitgeschrieben, damit ohne Netz die letzte bekannte Wahrheit gilt; Text, Auswahlgruppe und Größentafel hängen an demselben. (3) **B2:** die dritte Stufe ist weg, die Glasmenge wird ein sichtbares Angebot („Als Glas rechnen") über den Weg, den es dafür schon gibt. Dazu fünf C-Funde, darunter CSV und Druckblatt, die „Vom Fass" nicht kannten — das stand ausdrücklich im Plan dieser Runde und war liegen geblieben.
**Geprüft:** `npm test` **644/644** (vorher 634, 10 neue in `tests/groessen-schreiben.test.mjs`), neu `tests/ui-runde23.cjs` 11/11 im Browser — der A2-Fall lässt sich nur dort nachstellen (tippen, während eine Anfrage unterwegs ist). **Gegenproben:** die Reparatur zurückgebaut → 3 Prüfungen im Modell und 2 im Browser fallen. `ui-mass` (LAUF=runde-23b) alle Urteile grün, Gestaltungsschicht wortgleich; `ui-runde22` 31/31, `ui-runde21` 19/19, `ui-runde18` 54/54, `ui-runde17` 38/38, `ui-leitung-echt` 44/44, `ui-fremdgeraet` 10/10.
**Für die Nächsten:** Die drei A-Funde sind EIN Fehler in drei Verkleidungen: **erst senden, dann den eigenen Stand ändern.** Wer den Körper aus einem Schnappschuss baut, wer nach dem Senden neu zeichnet und wer eine Absage trotzdem übernimmt, macht dasselbe dreimal. Der eigene Stand gehört vor die Anfrage, und die Antwort entscheidet, ob er bleibt. Und: B2 ist die Lehre darüber. Eine Regel, die ein Mensch ausgesprochen hat, ist noch kein Recht, sie dort anzuwenden, wo die Angabe fehlt — „Glas Wein 125 ml" sagt nichts über einen Kassennamen, dem man nicht ansieht, ob er ein Glas meint.
**Phase/Thema:** B / Artikelgrößen, Fassbier — Nacharbeit
**Backlog:** neu **niedrig** — eigene Gestern-Rechnung statt `tagMinus`; zwei Quellen für die Fußzeile des Fass-Blocks; „alles hinterlegt", obwohl Glasmengen fehlen.
**STATUS:** VERBESSERUNGEN — **nicht gemergt**, die Migration ist nicht eingespielt.
