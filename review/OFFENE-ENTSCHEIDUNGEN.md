...
**Status:** ENTSCHIEDEN am 18.09.2026 (Runde 15), Text berichtigt am
19.09.2026 (Runde 16, Fund B5)
**Gemeldet:** Hauptsitzung, Runde 4
**Betrifft:** `src/index.js` (`PIN_LAENGE`), `public/index.html`
(`PIN_LAENGE`, `renderLogin`), `public/leitung.html` (`PIN_LAENGE`)

**Entschieden: genau vier Ziffern.** Das ist Option 2 der alten Liste, nur
mit vier statt sechs Stellen. Im Keller wird mit kalten Fingern und einer
Flasche in der anderen Hand getippt; vier Ziffern sind das, was sich ein
Team merkt, und die vierte Ziffer schickt von selbst ab — die
Bestätigungstaste (✓) bleibt als zweiter Weg, kostet aber keinen
Pflichttastendruck mehr.

**Der Preis, sauber benannt:** zehntausend Möglichkeiten statt einer
Million. Was dagegensteht, ist allein die Sperre, und sie ist deshalb
gestaffelt (zehn Fehlversuche je Viertelstunde, dann halbe, dann ganze
Stunde; gezählt über zwei Stunden je IP). Ein Angreifer schafft an einer
IP rund 370 Rateversuche am Tag.

**Die Zahl steht an genau drei Stellen, und alle drei heissen `PIN_LAENGE`:**
`src/index.js`, `public/index.html`, `public/leitung.html`. Seit Runde 16
leiten alle drei Muster, Feldlänge und Würfel daraus ab; die vier harten
Vieren im Backoffice sind weg. `tests/projektregeln.test.mjs` hält die
drei aufeinander — weicht eine ab, fällt der Lauf.

**ACHTUNG, hier stand bis Runde 16 das Gegenteil.** Der alte Text sagte
„Umgesetzt ist: sechs bis acht Ziffern" und empfahl, dabei zu bleiben. Das
war der Stand vom 17.09. und ist seit Runde 15 falsch. Wer danach handelte,
vergab sechsstellige Codes — der Worker antwortet darauf mit 422, und mit
einem solchen Code kommt niemand mehr herein. Der Satz ist deshalb hier
ersetzt und nicht nur ergänzt worden.
